...
function [result pcm time signal_one signal_zero] = libPCM2(bitstream, type_pcm, bit_duration, n_of_sample_b, V, verbose)

%libPCM           
%[result pcm time signal_one signal_zero] = libPCM(bitstream, type_pcm,
%                           bit_duration,n_of_sample_b, V, verbose)
...
senal_for_one = V*ones(1,n_of_sample_b);
senal_for_cero = -V*ones(1,n_of_sample_b);
...
if type_pcm == 2
   for i=1:length_bitstream
       if i == 1   %evaluation of first value
          ...
          last_value = 0;
          if pcm(length(pcm)) == V  %if last value of pcm[] is V, then...
              last_value = 1;
          else
              last_value = 0;
          end
...
%%4 UNIPOLAR-RZ
if type_pcm == 4
half_signal_one = [V*ones(1, n_of_sample_b/2) zeros(1, n_of_sample_b/2)];    
   for i=1:length_bitstream
       if bitstream(i) == 1
          pcm = [pcm half_signal_one];
       else
          pcm = [pcm zeros(1, n_of_sample_b)];
       end
   end
signal_one=half_signal_one;
signal_zero=zeros(1, n_of_sample_b);
end

%%5 BIPOLAR-RZ
if type_pcm == 5
half_signal_one = [V*ones(1, n_of_sample_b/2) zeros(1, n_of_sample_b/2)];    
half_signal_cero = [-V*ones(1, n_of_sample_b/2) zeros(1, n_of_sample_b/2)];
   for i=1:length_bitstream
       if bitstream(i) == 1
          pcm = [pcm half_signal_one];
       else
...
half_signal_cero = [zeros(1, n_of_sample_b)]; 
   for i=1:length_bitstream
       if bitstream(i) == 1
          pcm = [pcm [zeros(1, ceil(n_of_sample_b/4)) V*ones(1, n_of_sample_b/2) zeros(1, floor(n_of_sample_b/4))]];
          %rounding is done throught ceil for up and floor for down to
          %prevent non 4th order division numbers
          V=V*(-1); %para poder alternar la polaridad del V, tocó generar el vector en cada ciclo...
       else
          pcm = [pcm half_signal_cero];
       end
   end
...
%%7 BI-?-L
if type_pcm == 7
half_signal_one = [V*ones(1, n_of_sample_b/2) -V*ones(1, n_of_sample_b/2)];    
half_signal_cero = [-V*ones(1, n_of_sample_b/2) V*ones(1, n_of_sample_b/2)];
   for i=1:length_bitstream
       if bitstream(i) == 1
          pcm = [pcm half_signal_one];
       else
...
%%11 Dicode NRZ
if type_pcm == 11
half_signal_one = -V*ones(1, n_of_sample_b);
signal_cero=zeros(1, n_of_sample_b);
